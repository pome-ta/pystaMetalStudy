...
class Renderer:
  ...
  def _init(self, mtkView: MTKView) -> 'MTKViewDelegate':
    ...
    vertexDescriptor.attributes().objectAtIndexedSubscript_(
      0).name = 'position'  #'MDLVertexAttributePosition'
    vertexDescriptor.attributes().objectAtIndexedSubscript_(
      0).format = MDLVertexFormatFloat3
    ...
    '''
    asset.initWithURL_vertexDescriptor_bufferAllocator_preserveTopology_error_(
      assetURL, vertexDescriptor, allocator, True, err_ptr)
