...
# todo: あとで関数化
MTKMetalVertexDescriptorFromModelIO = c.MTKMetalVertexDescriptorFromModelIO
MTKMetalVertexDescriptorFromModelIO.argtypes = [ctypes.c_void_p]
MTKMetalVertexDescriptorFromModelIO.restype = ctypes.c_void_p
...
class Renderer:
  ...
  def _init(self, mtkView: MTKView) -> 'MTKViewDelegate':
    ...
    mesh = MTKMesh.alloc().initWithMesh_device_error_(mdlMesh, self.device,
                                                      err_ptr)
    ...
    v = MTKMetalVertexDescriptorFromModelIO(mesh.vertexDescriptor())
